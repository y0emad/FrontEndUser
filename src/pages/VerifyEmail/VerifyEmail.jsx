...
export default function VerifyEmail() {
    ...
    async function sendingData(values) {
        setIsLoading(true)
        try {
            let { data } = await axios.post("http://localhost:4000/auth/verify-email", values)
            console.log(data);
    ...
    let validationSchema = Yup.object({
        code:Yup.string().min(6, 'Code must be between 6 digits.').max(6, 'Code must be between 6 digits.').required("Code is required"),
